...
ssfr=np.arange(-12.625,-8.5,0.125)
digit1=np.digitize(data1[:,25],ssfr)
#digit2=np.digitize(data2[:,25],ssfr)

#DATAtmp=np.log10(np.power(10,data1[:,24])*np.power(10,data1[:,25]))
#ssfr=np.arange(-2.815,2.9,0.3)
#digit1=np.digitize(DATAtmp,ssfr)


#IND=np.arange(0,len(data1),1)

SFH1=np.zeros((32,16))
#SFH2=np.zeros((16,16))
ng1=np.zeros(32)
#ng2=np.zeros(16)
sSFRrange=np.zeros((32,2))
for i in range(1,33):
...
#	SFH2[i-1,:]=np.sum(dataTMP2[:,4:20],axis=0)/len(dataTMP2)
	...
	sSFRrange[i-1,0]=ssfr[i-1]
	sSFRrange[i-1,1]=ssfr[i]

DATA1=np.vstack((ng1,sSFRrange.T,SFH1.T)).T
#DATA2=np.vstack((ng2,sSFRrange.T,SFH2.T)).T

#np.savetxt('vespaBC_1_gals_{}_dr7_sSFR2binR.dat'.format(TYPE),DATA1)
#np.savetxt('vespaM05_2_gals_{}_dr7_sSFRbin.dat'.format(TYPE),DATA2)
