...
ng1=np.zeros(44)
#ng2=np.zeros(16)
MRrange=np.zeros((44,2))
s=0
for i in range(1,45):
		cut2=galmr_digit1==45-i
		dataTMP1=data1[cut2,:]
...
#                        SFH2[s,:]=np.sum(dataTMP2[:,4:20],axis=0)/len(dataTMP2)  
         	MRrange[s,0]=mr[45-i]
                MRrange[s,1]=mr[45-1-i]
		s=s+1

DATA1=np.vstack((ng1,MRrange.T,SFH1.T)).T
#DATA2=np.vstack((ng2,MRrange.T,SFH2.T)).T

np.savetxt('vespaBC_1_gals_{}_dr7_MR2binR.dat'.format(TYPE),DATA1)
#np.savetxt('vespaM05_2_gals_{}_dr7_MRbin.dat'.format(TYPE),DATA2)
